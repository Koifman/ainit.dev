...
export default async function handler(req) {
  ...
  const index = await fetch(new URL('/guardrails/index.json', base)).then(r => r.json());
  ...
  // Build output
  const rawQs = req.url.split('?').slice(1).join('?') || 'o=agents';
  let output = `# === ${outputFilename} ===\n# Generated by ainit.dev/guardrails\n# Technologies: ${slugs.join(', ')}\n# curl -L ainit.dev/api/guardrails/${slugs.join(',')}?${rawQs}\n\n`;
