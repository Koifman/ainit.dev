...
export function init() {
  ...
  async function load() {
    try {
      const res = await fetch('/gr/index.json');
  ...
  async function updatePreview() {
    ...
    const catContents = await Promise.all(activeCatSlugs.map(async slug => {
      ...
      const text = await fetchText(`/gr/categories/${slug}.md`);
    ...
    const techSections = await Promise.all(selectedTechs.map(async slug => {
      const text = await fetchText(`/gr/tech/${slug}.md`);
      const t = registry.technologies.find(x => x.slug === slug);
    ...
    if (activeFormat === 'AGENTS.md') output += `# AGENTS.md specification by https://agents.md\n`;
