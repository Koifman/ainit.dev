    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        if (req.url.startsWith('/api/init')) {
          ...
          let rulesContent = '';
          if (guardrails) {
            const grDir = path.resolve('public/gr');
            ...
            if (fmt.rules === 'AGENTS.md') output += `# AGENTS.md specification by https://agents.md\n`;
        ...
        if (req.url.startsWith('/api/guardrails/')) {
          ...
          const grCurlParts = [];
          if (catFilter) grCurlParts.push(`c=${catFilter}`);
          grCurlParts.push(`o=${formatParam || 'agents'}`);
          let output = `# === ${grOutputFilename} ===\n`;
          if (grOutputFilename === 'AGENTS.md') output += `# AGENTS.md specification by https://agents.md\n`;
          output += `# Generated by ainit.dev/guardrails\n# Technologies: ${slugs.join(', ')}\n# curl -L ainit.dev/api/guardrails/${slugs.join(',')}?${grCurlParts.join('?')}\n\n`;
        ...
        const header = `# === ${aiOutputFilename} ===\n# Templates: ${slugs.join(', ')}\n# curl -L ainit.dev/api/${slugs.join(',')}?o=${aiFormatParam || 'aiignore'}\n`;
        res.end(header + '\n' + parts.join('\n'));
      });
